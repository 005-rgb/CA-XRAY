...
async function collectEvmEvidence({ network, tokenAddress, pairAddress, decimals, timeoutMs = 12000 } = {}) {
  ...
  try {
    ...
    return {
      ...
      latestBlock: latest,
      exitability,
      holderHistory: { status: transferScan.complete ? "COMPLETE" : "PARTIAL", ...history, scannedFromBlock: transferScan.fromBlock, scannedToBlock: transferScan.toBlock, chunks: transferScan.chunks },
      ownerHistory: { status: ownerScan.complete ? "COMPLETE" : "PARTIAL", events: ownerEvents, scannedFromBlock: ownerScan.fromBlock, scannedToBlock: ownerScan.toBlock, chunks: ownerScan.chunks },
      limitations: [
        ...(transferScan.complete ? [] : ["Transfer event history is partial because the RPC imposed a range or response limit."]),
        ...(ownerScan.complete ? [] : ["Ownership event history is partial because the RPC imposed a range or response limit."]),
