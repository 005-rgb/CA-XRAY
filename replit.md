...
- The Arbitrum Immune System P1–P3 core path is implemented in `src/immune-system/`: intent and unsigned-transaction normalization, supported calldata decoding, permission exposure, deterministic policy decisions, Arbitrum One/Sepolia deployment evidence, bounded provenance, tenant-scoped decision idempotency, local attestation registry/admission-gate simulation, passport verification/invalidation, and deployment preflight. Demo fixtures are labeled `FIXTURE`/`LOCAL_SIMULATION`; they are not live chain deployments. P1–P3 tests run with `npm test`.
