...
function renderPassportArea() {
  ...
  const activeReviews = reviewQueue.filter((item) => item.status !== "RESOLVED");
  const reviewActions = (item) => item.status === "RESOLVED"
    ? `<button type="button" class="text-button" data-review-status="${escapeHtml(item.id)}" data-review-next="OPEN">Reopen</button>`
    : `<button type="button" class="text-button" data-review-status="${escapeHtml(item.id)}" data-review-next="ACKNOWLEDGED">Acknowledge</button><button type="button" class="text-button" data-review-status="${escapeHtml(item.id)}" data-review-next="DEFERRED">Defer</button><button type="button" class="text-button" data-review-status="${escapeHtml(item.id)}" data-review-next="RESOLVED">Resolve</button>`;
  const area = {
    overview: `<div class="passport-metric-grid">${passportMetric("Risk score", risk.score, scoreTone(risk.score))}${passportMetric("Reliability", current.reliabilityScore)}${passportMetric("Coverage", risk.coverage == null ? "UNKNOWN" : `${risk.coverage}%`)}${passportMetric("Health", freshness.healthScore == null ? "UNKNOWN" : `${freshness.healthScore}%`)}${passportMetric("Uncertainty budget", freshness.uncertaintyBudget == null ? "UNKNOWN" : `${freshness.uncertaintyBudget}%`)}${passportMetric("Posture", risk.score == null || freshness.uncertaintyBudget > 40 ? "INSUFFICIENT EVIDENCE" : risk.score >= 70 ? "HIGH PRIORITY REVIEW" : risk.score >= 45 ? "CONTINUE WITH CAUTION" : "MONITOR CLOSELY")}</div><div class="passport-callout"><strong>Evidence posture</strong><p>${risk.score == null ? "No reliable final risk score is available." : `Risk (${risk.score}) is separate from reliability (${current.reliabilityScore ?? "UNKNOWN"}), health (${freshness.healthScore ?? "UNKNOWN"}), and uncertainty (${freshness.uncertaintyBudget ?? "UNKNOWN"}%). This is not financial advice.`}</p></div>`,
    evidence: `<div class="passport-metric-grid">${passportMetric("Risk", risk.score)}${passportMetric("Reliability", current.reliabilityScore)}${passportMetric("Health", freshness.healthScore == null ? "UNKNOWN" : `${freshness.healthScore}%`)}${passportMetric("Uncertainty budget", freshness.uncertaintyBudget == null ? "UNKNOWN" : `${freshness.uncertaintyBudget}%`)}</div><div class="passport-table">${freshness.items?.length ? freshness.items.map((item) => `<div class="passport-evidence-row"><strong>${escapeHtml(item.id)}</strong><span class="evidence-state">${escapeHtml(item.freshness)}</span><small>${escapeHtml(item.family)} · ${escapeHtml(String(item.ageHours ?? "UNKNOWN"))}h old · TTL ${escapeHtml(String(item.ttlHours))}h · ${escapeHtml(item.status)}</small></div>`).join("") : evidence.length ? evidence.slice(0, 12).map((item) => `<div class="passport-evidence-row"><strong>${escapeHtml(item.label || item.id || item.key || "Evidence point")}</strong><span class="evidence-state">${escapeHtml(item.status || item.evidenceStatus || "UNKNOWN")}</span><small>${escapeHtml(item.evidenceId || item.source || "Reference unavailable")}</small></div>`).join("") : `<div class="unknown-message">No evidence points are available in this snapshot.</div>`}</div>`,
    ...
    review: `<div class="passport-metric-grid">${passportMetric("Open reviews", activeReviews.length)}${passportMetric("Open alerts", openAlerts.length)}${passportMetric("Monitoring", watch?.status || "NOT MONITORED")}${passportMetric("Next check", watch?.nextCheckAt ? formatDate(watch.nextCheckAt) : "Not scheduled")}</div><div class="passport-table">${activeReviews.length ? activeReviews.map((item) => `<div class="passport-review-row"><div class="passport-evidence-row"><strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.priority)} · ${escapeHtml(item.status)}</span><small>${escapeHtml(item.reason)}${item.evidenceRef ? ` · evidence ${escapeHtml(item.evidenceRef)}` : ""}</small></div><div class="passport-review-actions">${reviewActions(item)}</div></div>`).join("") : `<div class="unknown-message">No open evidence-backed review actions.</div>`}${openAlerts.length ? `<div class="passport-callout"><strong>Alerts</strong><p>${openAlerts.length} alert${openAlerts.length === 1 ? "" : "s"} also require review in Watchtower.</p></div>` : ""}</div>`,
    audit: `<div class="passport-metric-grid">${passportMetric("Evidence hash", current.evidenceHash ? current.evidenceHash.slice(0, 16) + "…" : "UNKNOWN")}${passportMetric("Engine version", current.engineVersion)}${passportMetric("Schema version", current.evidenceSchemaVersion)}${passportMetric("Immutable snapshots", timeline.length)}${passportMetric("Review events", reviewQueue.reduce((sum, item) => sum + (item.timeline?.length || 0), 0))}</div><div class="passport-table">${timeline.map((item) => `<div class="passport-evidence-row"><strong>${escapeHtml(item.id)}</strong><span>${escapeHtml(formatDate(item.capturedAt))}</span><small>Hash ${escapeHtml(item.evidenceHash || "UNKNOWN")} · replay-safe snapshot</small></div>`).join("")}</div><div class="passport-callout"><strong>Audit boundary</strong><p>Passport history is append-only. Export includes snapshots, hashes, freshness, and review events.</p><button type="button" class="secondary-button" id="passport-audit-export">Download audit package <span>↓</span></button></div>`,
  };
  ...
  passportAreaContent.querySelectorAll("[data-review-status]").forEach((button) => button.addEventListener("click", async () => {
    button.disabled = true;
    try {
      await apiJson(`/api/risk-passports/${encodeURIComponent(data.passport.networkId)}/${encodeURIComponent(data.passport.address)}/review-queue/${encodeURIComponent(button.dataset.reviewStatus)}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: button.dataset.reviewNext }),
      });
      await openPassportWorkspace(data.passport);
      document.querySelector(`[data-passport-area="${passportArea}"]`)?.click();
    } catch (error) { button.disabled = false; button.title = error.message; }
  }));
  passportAreaContent.querySelector("#passport-audit-export")?.addEventListener("click", async (event) => {
    event.currentTarget.disabled = true;
    try {
      const body = await apiJson(`/api/risk-passports/${encodeURIComponent(data.passport.networkId)}/${encodeURIComponent(data.passport.address)}/audit-export`);
      const blob = new Blob([JSON.stringify(body.auditPackage, null, 2)], { type: "application/json" });
      const link = document.createElement("a"); link.href = URL.createObjectURL(blob);
      link.download = `risk-passport-${data.passport.networkId}-${data.passport.address.slice(0, 10)}-audit.json`;
