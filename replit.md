...
- The Arbitrum Immune System Phase 0 product/evidence lock is complete in `doc/PHASE-0-IMMUNE-SYSTEM-LOCK.md`: the deterministic 500 USDC vault-deposit scenario, selector matrix, decision semantics, policy versions, reason codes, eligibility source, bounded provenance, fixture/live boundary, threat model, and evidence checklist are machine-readable in `src/immune-system/phase0.js`. Phase 1 transaction decoding and later on-chain capabilities are not implied by this lock.
- The Arbitrum Immune System P1–P3 core path is implemented in `src/immune-system/`: intent and unsigned-transaction normalization, supported calldata decoding, permission exposure, deterministic policy decisions, Arbitrum One/Sepolia deployment evidence with freshness, bounded provenance/conflict handling, tenant-scoped decision idempotency, canonical attestation hashing, local attestation registry/admission-gate simulation, passport verification/invalidation, and deployment preflight. Demo fixtures are labeled `FIXTURE`/`LOCAL_SIMULATION`; they are not live chain deployments. Run `npm test` and `npm run contracts:compile` for the reproducible checks. The P3 public-chain evidence package remains pending real deployment credentials and receipts.
...
- Phase 8 evidence community is available under `/api/community`: authenticated workspace researchers can maintain profiles, publish annotations only with evidence references, submit one independent peer review per annotation, raise evidence-backed disputes, and let authorized moderators approve/hide annotations or resolve/dismiss disputes. Reputation is quality-derived from citations, peer outcomes, and disputes; popularity is never used.
