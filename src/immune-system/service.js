...
class ImmuneSystemService {
  ...
  attestDecision({ workspaceId, decisionId, issuerId = "server", passportId = null } = {}) {
    const record = this.decisions.get(decisionId);
    ...
    if (record.attestation) return { duplicate: true, passport: this.registry.get(record.attestation.passportId) };
    ...
    return { duplicate: false, passport: clone(passport) };
  }

  getPassport(workspaceId, passportId) {
    const passport = this.registry.get(passportId);
    if (!passport || passport.workspaceId !== workspaceId) return null;
    return clone(passport);
  }

  verifyPassport({ workspaceId, passportId, subjectChainId = null, subject = null } = {}) {
    const passport = this.registry.get(passportId);
    if (!passport || passport.workspaceId !== workspaceId) return null;
    return this.gate.admit({ passportId, subjectChainId, subject });
  }

  invalidatePassport({ workspaceId, passportId, reasonCode = "DEPENDENCY_CHANGED" } = {}) {
    const passport = this.registry.get(passportId);
    if (!passport || passport.workspaceId !== workspaceId) throw Object.assign(new Error("PASSPORT_NOT_FOUND"), { code: "PASSPORT_NOT_FOUND" });
    return this.registry.invalidate({ issuerId: "server", passportId, reasonCode });
  }
